...
def main():
    ...
    # 初始化模型
    resnet50 = ResNet_50(num_class=7, device=device)
    resnet50 = resnet50.to(device)
    model = Model_10(num_class=7, device=device)
    ...
    for epoch in tqdm(range(start_epoch, epochs)):
        ...
        # 训练模型，并获取训练集上的准确率和损失
        train_acc, train_los = train(train_loader, resnet50, model, criterion_cls, optimizer, epoch+1)      
...
# 训练函数
def train(train_loader, teacher, student, criterion_cls, optimizer, epoch):
    ...
    for i, (images, targets) in enumerate(train_loader):
        ...
        out_teacher = teacher(images)
        out_student, heads = student(images)  # 前向传播，得到模型输出

        # 计算交叉熵损失
        loss1 = criterion_cls(out_student / temperature, out_teacher / temperature) * 10
        loss2 = criterion_cls(out_student, targets) * 10
        
        loss = loss1 + loss2

        acc = accuracy(out_student, targets)  # 计算准确率
